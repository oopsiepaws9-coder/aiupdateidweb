...
export default function AdvisorClient({tools}:{tools:AITool[]}){
 const[input,setInput]=useState(defaults); const[mode,setMode]=useState<IntelligenceProfile["mode"]>("balanced"); const[sent,setSent]=useState<Record<string,boolean>>({}); const[started,setStarted]=useState(false);
 ...
 const markStarted=(trigger:string)=>{if(started)return;setStarted(true);trackProductEvent("advisor_started","advisor",{trigger,task:input.task,mode});};
 const toggle=(key:keyof AdvisorInput["priorities"])=>{markStarted(`priority_${String(key)}`);setInput(c=>({...c,priorities:{...c.priorities,[key]:!c.priorities[key]}}));};
 async function feedback(toolId:string,helpful:boolean){if(sent[toolId])return;setSent(c=>({...c,[toolId]:true}));const result=results.find(item=>item.tool.id===toolId);trackProductEvent(helpful?"advisor_feedback_helpful":"advisor_feedback_unhelpful","advisor",{task:input.task,mode,tool_slug:result?.tool.slug||"unknown",decision_score:result?.total||0});try{await fetch("/api/advisor/feedback",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({taskKey:input.task,toolId,helpful,taskProfile:{...input.priorities,mode}})})}catch{}}
 return <div className={styles.shell}>
  <section className={styles.controls}>
   ...
   <div className={styles.taskGrid}>{TASKS.map(t=><button key={t.value} className={input.task===t.value?styles.taskActive:styles.task} onClick={()=>{markStarted("task");setInput(c=>({...c,task:t.value}));}}><strong>{t.label}</strong><span>{t.helper}</span></button>)}</div>
   <div className={styles.priorityBlock}><span className={styles.eyebrow}>STRATEGI KEPUTUSAN</span><div className={styles.pills}>{MODES.map(m=><button key={m.value} aria-pressed={mode===m.value} onClick={()=>{markStarted("mode");setMode(m.value);}}>{m.label}</button>)}</div></div>
  ...
  <section className={styles.results} aria-live="polite">
   ...
   {results.map((r,i)=><article className={i===0?styles.bestCard:styles.card} key={r.tool.id}><div className={styles.rank}>{i+1}</div><div className={styles.cardMain}>
    ...
    <div className={styles.actions}><Link href={`/tools/${r.tool.slug}`} onClick={()=>trackProductEvent("advisor_tool_open","advisor",{task:input.task,mode,tool_slug:r.tool.slug,rank:i+1,decision_score:r.total})}>Periksa profil & keterbatasan <ChevronRight size={16}/></Link><div className={styles.feedback}><span>{sent[r.tool.id]?"Tersimpan":"Hasil ini cocok?"}</span><button disabled={sent[r.tool.id]} onClick={()=>feedback(r.tool.id,true)} aria-label={`Rekomendasi ${r.tool.name} membantu`}><ThumbsUp size={15}/></button><button disabled={sent[r.tool.id]} onClick={()=>feedback(r.tool.id,false)} aria-label={`Rekomendasi ${r.tool.name} tidak membantu`}><ThumbsDown size={15}/></button></div></div>
   </div></article>)}
